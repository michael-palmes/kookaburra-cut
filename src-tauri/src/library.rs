//! The template and preset libraries: a template is a whole project folder carrying `template.json`, a preset is a single-scene project folder carrying `preset.json`, and both live either in the checkout (`projects/`, `presets/`) or in the user's workspace (`templates/`, `presets/`). These commands only move folders and text: every manifest schema is owned by the frontend (`src/engine/workspace/templates.ts`, `src/engine/workspace/presets.ts`), exactly as themes and export presets are. Writes into the CHECKOUT are dev-only (`#[cfg(debug_assertions)]`, registered conditionally in `lib.rs`), so a release binary carries no repo-write surface at all.
