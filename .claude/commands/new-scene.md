...
4. Write the sidecar `projects/$1/scenes/<stem>.json` per the skill's schema: `version: 1`,
   ...
   - device/deviceonly: one `devices[0]` entry (`id: "d1"`, catalog `model`/`colour`,
     `media` if given, `motion: { "preset": "none" }`). device: `placement` position
     `[0, -0.3, 0]`, `rotationDeg: [0, 0, 0]`, `scale: 1`, `shadow: "soft"`. deviceonly:
     position `[0, 0, 0]` (no title to clear), `scale: 1.35` (dominant framing),
     `ground: true` (rests on a staged floor when the theme has one) and NO `shadow` key,
     so Device auto-resolves it (map shadows over a floor, soft blob when floating).
