...
8. **Route machine-editable values through the scene document** (`scenes/<stem>.json` beside the TSX, see "Scene documents" in `REFERENCE.md`). Text has two explicit ownership modes. A code-owned scene has no `managedText`; its user-visible scene copy lives in `text` and is read with `useSceneText(key, fallback)`. A new inspector-owned scene carries authoritative flat `managedText.items`; optional `managedText.groups` references those leaf keys to make each Content Text row atomic and independently aligned. A group may contain repeated Icon, Title, Subtitle and Bullets leaves in its declared order. A block without groups resolves as one implicit group without a load-time rewrite. `text` remains only a compatibility mirror or a home for embedded labels. Scaffolded templates set `managedText.layout: "template"` so the existing composition renders the managed items. A structural inspector edit removes that layout flag and moves the scene to the generic responsive stack. Mark positioned composition copy such as comparison labels with `useSceneText(key, fallback, "embedded")` and `managedTextRole="embedded"` so it never becomes a managed line. Scaffolded device scenes read their devices from `useSceneDevices()`; edit the sidecar (managed text, device model/colour/media, motion, shadow, **camera track** (orbit keys/segments, see "Per-scene camera tracks" in `REFERENCE.md`), the v8 staging fields **`themeId`/`backdrop`/`lighting`**, the scene's display **`name`**, and the v11 fields **`background`** (camera-locked fill: colour/gradient/image, or a looping VIDEO since v12 · M4, scene-doc only) and **`textAnimation`** (whole-spec text motion, what the app's Text-motion panel writes; explicit TSX preset props override it unless `textAnimationForce: true`, the panel's Override, so prefer sidecar-driven motion on scaffolded scenes) and **`textLook`** (whole-spec text style, what the Text-style panel writes; the same force pattern via `textLookForce`, per-element via `textLookOverrides`, see "Text looks (Text style)" in `REFERENCE.md`), and the **`layeredScreenshot`** block + **`animatedTrack`** (the 3D screenshot stack and which ONE keyed track animates the scene, see "Layered screenshot" in `REFERENCE.md` for the schema, terminal recipes and inspector parity), and the **`frame`** block (the scene overlay: a camera-locked panel with a shaped cutout the scene renders through, see "Scene overlays (the frame block)" in `REFERENCE.md`), and the v14 **`chart`** block (one data visual per scene: eight types, 2D/3D, appearance + build-in presets, keyed data; the TSX is one `<Chart />` line and every knob lives in the sidecar. **Load `CHARTS.md` beside this skill before authoring or editing any chart**; `scripts/chart.py` handles data import, keyframes and a summary)), not the TSX, for those changes. The TSX stays the composition. When you change a `follow-media` scene's video source in the sidecar, also update the scene's `durationMs` in `project.json` to the new video's length (ffprobe). A scene without a sidecar is legal; it simply shows no editing affordances.
...
python3 .claude/skills/kookaburra-scene-authoring/scripts/sidecar.py 01-hero get managedText
