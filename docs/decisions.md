...
| Depth of field rendering (2026-08-04) | Stock `postprocessing` DepthOfField/TiltShift effects at the composer chain's head, built by a project-stable union carried on the camera plan; per-frame CPU-written uniforms; transition/compare sides render through a dof-only side composer into their HDR targets before the composite; display constants are rebase-class | Depth mode needs linear HDR + depth pre-tone-map, and the chain must stay project-stable (no mid-project recompiles). Per-side grading keeps a rack riding INTO a crossfade; a composite-frame dof would blur side B by side A's depth |
