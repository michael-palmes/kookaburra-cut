...
| Scene terminals | The `terminal` sidecar block is interactive DOM in preview and Present but exports its captured snapshot (a styled-run grid in the sidecar plus a baked PNG, the export truth); the start command is PRE-TYPED, never auto-run; scene sessions may opt out of the F-006 workspace cwd confinement to open a user-chosen start path (the path must still exist; interactive shells only, system directories refused) | `gl.readPixels` never sees DOM, so interactivity carries zero determinism risk; a `.kbpack` that auto-ran a command on slide view would be arbitrary code execution, while a start path runs nothing of the pack's (the login shell still runs the user's own init files, which is why a custom path never auto-spawns in Present: the first click starts it) |
| Scene websites (2026-08-29) | The `website` sidecar block uses a native child WKWebView only after per-project top-level-origin approval in the paused editor and during slideshow holds; playback, transitions and export render a manually captured PNG in deterministic WebGL chrome; remote views have no Tauri capability, device/file permissions or autoplay; one app-wide Website data store is separate from Safari, while grants never travel in packs | A remote page is untrusted code that cannot join `gl.readPixels`; an explicit native boundary preserves ordinary login flows without making project open/import a network action or allowing a shared project to inherit authority |
| Website platform floor (2026-08-29) | Kookaburra Cut requires macOS 26 once Website content ships; the child-view adapter exact-pins its Tauri/Wry boundary and gates in the packaged branch-built app | Named persistent Website storage and the required current WebKit controls are more valuable than carrying an older, weaker native path |
| Start command enforcement | The pre-typed command is reduced to a single line with control/format chars stripped (`sanitizeStartCommand`, at parse and again at the paste boundary), NOT left to bracketed-paste mode | xterm turns `\n` into Enter and only brackets a paste once the shell enables the mode, so a newline in an imported pack's command could auto-run; a data field must never execute shell |
| Terminal session ownership (2026-09-07) | Every session command (`pty_write`, `pty_resize`, `pty_kill`, `pty_pause`, `pty_resume`) refuses a session another window spawned; the F-006 cwd confinement is a pure function with its boundary cases under test | Makes the owner tag every session already carried load-bearing and blocks a cross-window write; hygiene, not the boundary: the F-001 trust gate is, since scene code runs in the webview with IPC reach and a hostile scene in the main window would be writing to its own session |
| Import sources (2026-09-07) | `import_object` and `import_global_screenshots` copy from whichever absolute path the file dialog returned, bounded by an extension allow-list, and are deliberately NOT confined to the workspace roots the way `hash_file` and `reveal_in_finder` are | Importing a .glb or a screenshot from Downloads is the point; the copy lands inside the workspace, the path is user-picked, and the F-001 trust gate bounds what a scene could do with the result |
...
| Editor render timing | Each source video is normalised to the output rate once and split per use before any clip trims it, and every clip is padded and cut to its frame count on the cumulative timeline grid; a variable-frame-rate source (Simulator recordings) seeds a 60 fps edit | Trimming a VFR file directly dropped the held frame at the in-point and leaked the last frame's hold past the out-point, so concat drifted by seconds and tap highlights missed their moment; the average rate of a VFR recording is not a cadence |
...
| Foldables (2026-09-19) | A primary `screen` (the inside display) plus an optional `coverScreen`, with `media` and `coverMedia` to match, rather than a list of screens; the fold is its own `foldDeg`, separate from `lidDeg`; the glb carries a clip keyed one frame per degree with the camera half baked static, sampled directly with no mixer | `media` is read at about twenty five sites and the inside display matches the open body's aspect, so the smallest change is also the honest one. A per-degree clip turns posing into a lookup, keeps the vendor's non-linear hinge slide, and makes a frame a pure function of the angle. Full contract: `docs/foldables.md` |
| Foldable screen power (2026-09-19) | Automatic by hinge angle (outside lit closed, inside lit open, a smoothstep handover around a switch angle) with a per-device "keep both on" override; an empty display is black | It is what the hardware does, so an untouched unfold looks right; the override covers book and tent poses that show both |
| Foldable blur handover (2026-09-19) | The moving panel's content is projected flat from a fixed front eye onto the display's home plane, then blurred and dimmed up one ramp measured off Apple's footage, in a single pass patched onto the foldable screen materials. It plays only while a fold animation carries the device across the power switch, on its progress through that fold; at rest every pose is lit and sharp. No render target, no mipmaps | The flat look is the point of Apple's effect, and a fixed eye is all the hardware could know, so content never swims as a shot orbits. Playing on the bare angle would leave a held Book or Flex pose half blurred for ever; running on the fold in progress keeps it a pure function of the track and the frame time, reverses on closing, and lands clean wherever the fold stops. Staying off render targets keeps the frame on the direct path and legacy projects structurally untouched; clip frames have no mip chain, and adding one would change how the sharp path samples |
