...
import { releaseComposer } from "../effects/effects";
import {
...
import { collectEnvironmentSources, preloadEnvironments } from "../lighting/environments";
import { type LoadedProject, loadProject, previewLabProjectIds, sceneFileStem } from "../project";
import { libraryPreviewFormat, resolveLibraryPreviewPoint } from "../workspace/libraryPreviewPoint";
import { listPresets, presetPreviewFrame } from "../workspace/presets";
import { findTemplate, listTemplates } from "../workspace/templates";
import { createProject } from "../workspace/workspace";
...
function parseAspects(raw: string | undefined): FormatSpec[] {
  ...
  // "all" = the STANDING matrix (16:9, 9:16, 1:1); 4:5 / 5:4 / 3:2 / 2:3 and the phone pair are first-class but their gates stay feature-scoped, so they must be requested explicitly.
  if (value === "" || value === "all") return STANDING_ASPECTS.map((a) => FORMATS[a]);
  const spec = FORMATS[value as AspectName];
  if (!spec) {
    throw new Error(
      `unknown KOOKABURRA_ASPECT "${value}" (expected 16:9 | 9:16 | 1:1 | 4:5 | 5:4 | 3:2 | 2:3 | phone | phone-landscape | all)`,
    );
  }
  return [spec];
}
        ...
        if (encode && config.loudnessTarget !== undefined && current.audio) {
          ...
          const measured = await invoke<{ integratedLufs: number; truePeakDbtp: number }>(
            "measure_loudness",
            {
              ...
              fadeOutCurve: current.audio.fadeOutCurve ?? null,
              startOffsetMs: current.audio.startOffsetMs ?? 0,
              trackDurationMs: Math.round(current.audio.durationMs),
