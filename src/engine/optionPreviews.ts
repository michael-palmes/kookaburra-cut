...
/** Map preview-lab's scene stems to capture jobs (pure; the autorun action and its tests share it): `tm-<preset>` scenes render text-motion CLIPS (except `tm-none`, which is motionless, so one still is honest); `tl-<preset>` scenes render text-look CLIPS (a plain fade, the hover-play card); `bg-<shader>` scenes render animated-background CLIPS; `chartanim-<preset>` scenes render chart build-in CLIPS (the motion IS the option); `bgp-<shader>-<preset>` scenes render shader-preset STILLS (small tiles, motion already shown by the type card); `shadow-*` / `stage-*` / `object-*` (object-picker cards) / `chart-*` (chart appearance cards, a settled chart) scenes are stills. Unknown stems are skipped, so lab experiments never break the batch. */
