...
/** Chart data-lane UI state: the selected key/segment, following the compareEditStore pattern (the staged chart's gizmo state lives in `chartEditStore.ts`). UI-only, the export path never reads it, and unlike the camera and divider stores it holds no live draft: the chart renders straight from the scene document, so lane edits show once the commit patches the doc (a draft merge would have to sit on the shared render path the export loop also walks). */
